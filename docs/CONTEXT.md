...
### LangGraph
A state-machine framework for LLM-driven workflows. We adopt it as the chatbot orchestrator (see ADR-002). The chatbot is a compiled `StateGraph` with `agent` and `tools` nodes plus one conditional edge that decides whether to keep looping.

### StateGraph
The LangGraph builder we instantiate in `chat/graph.py`. Calls `add_node`, `add_edge`, `add_conditional_edges`, then `.compile()` to produce a `GRAPH` singleton invoked by the Streamlit UI.

### Node
A function that takes a `ChatState` and returns a dict of state updates. Our two nodes: `agent_node` (calls the LLM with bound tools) and `tool_node` (LangGraph's prebuilt `ToolNode` over `ALL_TOOLS`).

### Conditional edge
An edge whose destination is decided by a routing function on the current state. Our routing function `route_after_agent` returns `"tools"` if the LLM's last `AIMessage` carries `tool_calls`, otherwise `"end"`.

### ChatState
The `TypedDict` defining what flows through the graph: `messages` (with LangGraph's `add_messages` reducer), `user_id`, `conversation_id`, `tokens_used`.

### `add_messages` reducer
LangGraph's standard reducer for the `messages` channel. New messages are appended to history rather than replacing it. Without this annotation, each node would overwrite the message list — almost always a bug.

### `@tool` decorator
From `langchain_core.tools`. Wraps a Python function as a callable tool exposed to the LLM. We use it in `chat/tools/<domain>.py` files to wrap domain functions; the docstring becomes the tool description shown to the model, and `args_schema=<PydanticModel>` defines the input contract.

### ToolNode
LangGraph's prebuilt node that executes any tool calls in the last `AIMessage` and returns `ToolMessage` objects with results. Handles parallel tool calls correctly out of the box.

### ChatAnthropic
The `langchain-anthropic` model wrapper around Anthropic's SDK. We call `.bind_tools(ALL_TOOLS)` once at module load; the bound model auto-includes tool schemas on every `.invoke()`.

### BaseCallbackHandler
The LangChain hook for observing model and tool events. Our two implementations: `DuckDBAuditCallback` (HR-5 audit trail) and `TokenBudgetCallback` (HR-7 cost guardrail). Both pass through `config={"callbacks": [...]}` on `GRAPH.invoke`.

### MemorySaver
LangGraph's in-process checkpointer. v1 uses it; cross-session resume (persistent checkpointer) is post-v1.
...
| "LangChain" alone | Be specific: "LangGraph" (orchestrator), "`langchain-core`" (tool decorator), or "`langchain-anthropic`" (model wrapper). We do NOT use the `langchain` meta-package — saying "LangChain" suggests we did. |
